...
int creatEventfd()
{
    ...
    if(evtfd<0){
        LOG_FATAL("eventfd errno:%d",errno);
    }
    return evtfd;
}
EventLoop::EventLoop()
...
void EventLoop::queueInLoop(Functor cb)
{
    ...
    //就是说：如果当前线程不是事件循环所在的线程，或者正在调用挂起的回调函数，
    //那么就调用wakeup函数来唤醒事件循环。这样，事件循环就可以尽快地处理新添加的回调函数。
...
//这样做有两个好处：一方面减小了临界区的长度（不会阻塞其他线程调用queueInLoop()），另一方面避免了死锁（因为Functor可能再调用queueInLoop()）1。